...
def update_folder_names_and_paths(key, targets=None):
    if targets is None:
        targets = []
...
def resolve_full_path(name, keys):
    for key in keys:
...
class CLIPLoaderGGUF:
    ...
    @staticmethod
    def resolve_clip_path(name):
        keys = (
            ("clip_gguf", "clip")
            if name.lower().endswith(".gguf")
            else ("clip", "clip_gguf")
        )
        return resolve_full_path(name, keys)
    ...
    def load_clip(self, clip_name, type="stable_diffusion"):
        clip_path = self.resolve_clip_path(clip_name)
        if clip_path is None:
            raise FileNotFoundError(f"Unable to find CLIP file: {clip_name}")
        clip_type = getattr(
...
class DualCLIPLoaderGGUF(CLIPLoaderGGUF):
    ...
    def load_clip(self, clip_name1, clip_name2, type):
        clip_path1 = self.resolve_clip_path(clip_name1)
        if clip_path1 is None:
            raise FileNotFoundError(f"Unable to find CLIP file: {clip_name1}")
        clip_path2 = self.resolve_clip_path(clip_name2)
        if clip_path2 is None:
            raise FileNotFoundError(f"Unable to find CLIP file: {clip_name2}")
...
class TripleCLIPLoaderGGUF(CLIPLoaderGGUF):
    ...
    def load_clip(self, clip_name1, clip_name2, clip_name3, type="sd3"):
        ...
        if clip_path2 is None:
            raise FileNotFoundError(f"Unable to find CLIP file: {clip_name2}")
        clip_path3 = self.resolve_clip_path(clip_name3)
        if clip_path3 is None:
            raise FileNotFoundError(f"Unable to find CLIP file: {clip_name3}")
...
class QuadrupleCLIPLoaderGGUF(CLIPLoaderGGUF):
    ...
    def load_clip(
        ...
        if clip_path3 is None:
            raise FileNotFoundError(f"Unable to find CLIP file: {clip_name3}")
        clip_path4 = self.resolve_clip_path(clip_name4)
        if clip_path4 is None:
            raise FileNotFoundError(f"Unable to find CLIP file: {clip_name4}")
